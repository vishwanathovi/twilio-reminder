...
class ReminderService:
    ...
    def run(self):
        ...
        try:
            while self.running:
                ...
                if (
                    current_time - self.last_upcoming_log_time
                    >= self.upcoming_log_interval
                ):
                    self._log_upcoming_reminders()
        ...
        try:
            ...
            if due_reminders:
                ...
                for reminder in due_reminders:
                    logger.info(
                        f"Executing reminder ID: {reminder['id']} for {reminder['user_name']}"
                    )
    ...
    def _log_next_reminder(self, reminders):
        ...
        try:
            ...
            if upcoming:
                ...
                logger.info(
                    f"Next reminder: {reminder['user_name']} - \"{reminder['content']}\" at {next_time} (in {time_str})"
                )
            else:
    ...
    def _log_upcoming_reminders(self):
        ...
        try:
            ...
            logger.info("=" * 70)
            logger.info(
                f"UPCOMING REMINDERS (Next 24 Hours): {len(upcoming)} reminder(s)"
            )
            logger.info("=" * 70)
